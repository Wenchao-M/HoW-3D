...
class PositionEmbeddingSine_3D(nn.Module):
    ...
    def forward(self, x):
        ...
        b, c, h, w = x.shape
        assert h==w
        not_mask = torch.ones((b, h, w, w), dtype=torch.uint8, device=x.device)
...
def build_position_encoding_3D(position_embedding_mode='sine', hidden_dim=256):
    assert hidden_dim % 3 == 0
    N_steps = hidden_dim // 3
    if position_embedding_mode in ('v2', 'sine'):
        # TODO find a better way of exposing other arguments
        position_embedding = PositionEmbeddingSine_3D(N_steps, normalize=True)
    else:
        raise ValueError(f"not supported {position_embedding_mode}")
